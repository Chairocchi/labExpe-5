<?php
	...
	if($_GET['type'] == 'studentname'){
		...
		while ($row = $result->fetch_assoc()) {
			//array_push($data, $row['name'].'-'.$row['contact']);	
	...
	if($_GET['type'] == 'report'){
		$result = $conn->query("SELECT name,contact FROM student where (name LIKE '%".$_GET['name_startsWith']."%' or contact LIKE '%".$_GET['name_startsWith']."%')   ");	
		$data = array();
		while ($row = $result->fetch_assoc()) {
			//array_push($data, $row['name'].'-'.$row['contact']);	
			array_push($data, $row['name']);	
